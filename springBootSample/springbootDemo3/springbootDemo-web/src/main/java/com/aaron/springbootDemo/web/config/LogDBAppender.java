...
public class LogDBAppender extends DBAppenderBase<ILoggingEvent> {
    ...
    private static String buildInsertSQL() {
        return "INSERT INTO `t_tw_logging`(`message`,`level_string`,`created_time`,`logger_name`)" +
                "VALUES (?,?,?,?)";
    }
...
//            System.out.println("-------------------------------------------------------------------------");
//            System.out.println(event.getThrowableProxy().getClassName()+Arrays.toString(event.getThrowableProxy().getStackTraceElementProxyArray()));
//            System.out.println("-------------------------------------------------------------------------");
//         }
//        if(event.getCallerData() != null){
//            System.out.println(Arrays.toString(event.getCallerData()));
//        }


        // 如果只想存储自己打印的日志，可以这样写日志：logger.info("- XXXX")
        String stackTrace = event.getThrowableProxy() != null?event.getThrowableProxy().getClassName()+Arrays.toString(event.getThrowableProxy().getStackTraceElementProxyArray()):"";
        stmt.setString(MESSAGE, message+"---"+stackTrace);
